...
// WebGL渲染常量
const RENDER_CONFIG = {
  MAX_POINTS: 1000000, // GPU缓冲区预算
  MAX_ROBOTS: 10, // 最大机器人数
  ROBOT_BUFFER_SIZE: 100000, // 单机器人缓冲
  POINT_SIZE: 5.0, // 点大小
  LINE_WIDTH: 2.5, // 线宽
...
export class WebGLRenderer {
  constructor(canvas) {
    ...
    // 机器人路径管理器
    this.robotManager = new RobotPathManager(10, 100000);
  ...
  initMarkerBuffers() {
    ...
    gl.vertexAttribPointer(
      this.markerAttributes.position,
      2,
      gl.FLOAT,
      false,
      28,
  ...
  updateTransform(transform) {
    this.transform.scale =
      transform && typeof transform.scale === 'number' ? transform.scale : 1.0;
    this.transform.translateX =
      transform && typeof transform.x === 'number' ? transform.x : 0;
    this.transform.translateY =
      transform && typeof transform.y === 'number' ? transform.y : 0;
  }
   ...
   */
  updatePoints(data) {
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.time('[PERF_DEGRADE] updatePoints');
    const dataView = new DataView(data);
    const pointCount = dataView.getUint32(0, true);

    if (this.debug) {
      console.log('WebGL: 接收点数据', {
        pointCount,
    ...
    if (pointCount === 0) {
      if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
        console.timeEnd('[PERF_DEGRADE] updatePoints');
      return;
    }
    ...
    if (this.debug) {
      ...
      if (this._updateCount % 20 === 0) {
        console.log('WebGL: 路径边界', {
          minX: this.dataBounds.minX.toFixed(0),
          maxX: this.dataBounds.maxX.toFixed(0),
          minY: this.dataBounds.minY.toFixed(0),
          maxY: this.dataBounds.maxY.toFixed(0),
  ...
  uploadBufferData() {
    ...
    gl.bufferSubData(
      gl.ARRAY_BUFFER,
      0,
      this.ringBuffer.vertices.subarray(0, this.ringBuffer.pointCount * 2),
    );
    gl.bindBuffer(gl.ARRAY_BUFFER, this.buffers.color);
    ...
    for (let i = 0; i < this.ringBuffer.pointCount * 4; i++) {
      colorSlice[i] = Math.min(
        255,
        Math.max(0, Math.round(this.ringBuffer.colors[i] * 255)),
      );
    }
  ...
  _packRobotsToStaging() {
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.time('[PERF_DEGRADE] _packRobotsToStaging');
    ...
    if (total > this.MAX_POINTS) {
      perCap = Math.max(
        1,
        Math.floor(this.MAX_POINTS / Math.max(1, robots.length)),
      );
    }
    let write = 0;
    for (const r of robots) {
      ...
      if (count <= 0) {
        this._packedDraws.push({ start: write, count: 0 });
        continue;
      }
      ...
      // 位置
      this._stagingPos.set(
        r.vertices.subarray(startIndex, endIndex),
        write * 2,
      );
      // 颜色（float -> u8）
      ...
      for (let i = 0; i < keep; i++) {
        ...
        const dst = (write + i) * 4;
        this._stagingColorU8[dst] = Math.min(
          255,
          Math.max(0, Math.round(r.colors[src] * 255)),
        );
        this._stagingColorU8[dst + 1] = Math.min(
          255,
          Math.max(0, Math.round(r.colors[src + 1] * 255)),
        );
        this._stagingColorU8[dst + 2] = Math.min(
          255,
          Math.max(0, Math.round(r.colors[src + 2] * 255)),
        );
        this._stagingColorU8[dst + 3] = Math.min(
          255,
          Math.max(0, Math.round(r.colors[src + 3] * 255)),
        );
      }
    ...
    this._needsPack = false;
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.timeEnd('[PERF_DEGRADE] _packRobotsToStaging');
  }
  ...
  renderRobotPaths(totalPoints = 0) {
    ...
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.time(`[PERF_DEGRADE] renderRobotPaths${logSuffix}`);
    const gl = this.gl;
    const robots = this.robotManager.getActiveRobots();
    if (robots.length === 0 || !this.dataBounds.initialized) {
      if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
        console.timeEnd(`[PERF_DEGRADE] renderRobotPaths${logSuffix}`);
      return;
    }
    ...
    gl.uniform2f(
      this.uniforms.dataMin,
      this.dataBounds.minX,
      this.dataBounds.minY,
    );
    gl.uniform2f(
      this.uniforms.dataMax,
      this.dataBounds.maxX,
      this.dataBounds.maxY,
    );
    gl.uniform2f(
      this.uniforms.viewTranslate,
      this.transform.translateX * dpr,
      this.transform.translateY * dpr,
    );
    ...
    gl.bindVertexArray(this.vao);
    if (this._needsPack) {
      this._packRobotsToStaging();
      this._gpuDirty = true;
    }
    if (this._gpuDirty && this._packedTotal > 0) {
      if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
        console.time(`[PERF_DEGRADE] GPU Upload${logSuffix}`);
      gl.bindBuffer(gl.ARRAY_BUFFER, this.buffers.position);
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        0,
        this._stagingPos.subarray(0, this._packedTotal * 2),
      );
      gl.bindBuffer(gl.ARRAY_BUFFER, this.buffers.color);
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        0,
        this._stagingColorU8.subarray(0, this._packedTotal * 4),
      );
      this._gpuDirty = false;
      if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
        console.timeEnd(`[PERF_DEGRADE] GPU Upload${logSuffix}`);
    }
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.time(`[PERF_DEGRADE] Draw Calls${logSuffix}`);
    ...
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.timeEnd(`[PERF_DEGRADE] Draw Calls${logSuffix}`);
    gl.bindVertexArray(null);
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.timeEnd(`[PERF_DEGRADE] renderRobotPaths${logSuffix}`);
  }
  ...
  renderMarkers(time = 0) {
    ...
    // 设置uniforms
    gl.uniform2f(
      this.markerUniforms.resolution,
      this.canvas.width,
      this.canvas.height,
    );
    gl.uniform2f(
      this.markerUniforms.dataMin,
      this.dataBounds.minX,
      this.dataBounds.minY,
    );
    gl.uniform2f(
      this.markerUniforms.dataMax,
      this.dataBounds.maxX,
      this.dataBounds.maxY,
    );
    gl.uniform2f(
      this.markerUniforms.viewTranslate,
      this.transform.translateX,
      this.transform.translateY,
    );
   ...
   */
  render() {
    const totalPoints = this.robotManager
      ? this.robotManager.getTotalPoints()
      : 0;
    const logSuffix = ` | totalPoints: ${totalPoints}`;
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.time(`[PERF_DEGRADE] render${logSuffix}`);
    const gl = this.gl;
    ...
    if (typeof window !== 'undefined' && window.__PERF_VERBOSE__)
      console.timeEnd(`[PERF_DEGRADE] render${logSuffix}`);
  }
  ...
  renderRingBuffer() {
    ...
    const maxX = this.dataBounds.initialized
      ? this.dataBounds.maxX
      : this.canvas.width;
    const maxY = this.dataBounds.initialized
      ? this.dataBounds.maxY
      : this.canvas.height;
    gl.uniform2f(
      this.uniforms.resolution,
    ...
    gl.uniform2f(
      this.uniforms.viewTranslate,
      this.transform.translateX * dpr2,
      this.transform.translateY * dpr2,
    );
    gl.uniform1f(this.uniforms.viewScale, this.transform.scale);
    gl.uniform1f(this.uniforms.pointSize, this.pointSize);
  ...
  renderTestPoints() {
    ...
    for (let i = 0; i < testColors.length; i++) {
      testColorsU8[i] = Math.min(
        255,
        Math.max(0, Math.round(testColors[i] * 255)),
      );
    }
    ...
    if (this._testFrameCounter % 100 === 0) {
      console.debug('WebGL: 测试点渲染', {
        ...
        testVertices: {
          ...
          horizontalLine: `(100, ${currentHeight / 2}) to (${
            currentWidth - 100
          }, ${currentHeight / 2})`,
          verticalLine: `(${currentWidth / 2}, 100) to (${currentWidth / 2}, ${
            currentHeight - 100
          })`,
        },
      });
  ...
  resize(width, height) {
    ...
    if (this.gl) {
      ...
      console.debug('WebGL: Canvas调整大小', {
        cssWidth: width,
        cssHeight: height,
        dpr,
        drawingBuffer: { w: dw, h: dh },
      });
    }
  }
